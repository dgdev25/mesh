...
run Mesh's planner with two separate ideas. Then when it's done, use Mesh's `consensus` tool to pass the entire
...
```
Use mesh's planner and show me how to add real-time notifications to our mobile app
```
...
```
Create a plan using mesh for migrating our React app from JavaScript to TypeScript
```

```
Develop a plan using mesh for implementing CI/CD pipelines across our development teams
```
...
Like all other tools in Mesh, you can `continue` with a new plan using the output from a previous plan by simply saying

```
Continue with mesh's consensus tool and find out what o3:for and flash:against think of the plan 
```
