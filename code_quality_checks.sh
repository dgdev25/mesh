...
# Mesh MCP Server - Code Quality Checks
...
echo "🔍 Running Code Quality Checks for Mesh MCP Server"
echo "================================================="

# Determine Python command
if [[ -f ".mesh_venv/bin/python" ]]; then
    PYTHON_CMD=".mesh_venv/bin/python"
    PIP_CMD=".mesh_venv/bin/pip"
    echo "✅ Using venv"
...
for tool in ruff black isort pytest; do
    ...
    if [[ -f ".mesh_venv/bin/$tool" ]] || command -v $tool &> /dev/null; then
        continue
    else
...
# Set tool paths
if [[ -f ".mesh_venv/bin/ruff" ]]; then
    RUFF=".mesh_venv/bin/ruff"
    BLACK=".mesh_venv/bin/black"
    ISORT=".mesh_venv/bin/isort"
    PYTEST=".mesh_venv/bin/pytest"
else
    RUFF="ruff"
    BLACK="black"
    ISORT="isort"
...
$RUFF check --fix --exclude test_simulation_files --exclude .mesh_venv

echo "🎨 Running black code formatting..."
$BLACK . --exclude="test_simulation_files/" --exclude=".mesh_venv/"

echo "📦 Running import sorting with isort..."
$ISORT . --skip-glob=".mesh_venv/*" --skip-glob="test_simulation_files/*"

echo "✅ Verifying all linting passes..."
$RUFF check --exclude test_simulation_files --exclude .mesh_venv
