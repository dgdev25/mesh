...
def test_error_listing_respects_env_restrictions(monkeypatch, reset_registry):
    ...
    monkeypatch.setenv("MESH_MCP_FORCE_ENV_OVERRIDE", "false")
    env_config.reload_env({"MESH_MCP_FORCE_ENV_OVERRIDE": "false"})
    try:
        import dotenv

        monkeypatch.setattr(dotenv, "dotenv_values", lambda *_args, **_kwargs: {"MESH_MCP_FORCE_ENV_OVERRIDE": "false"})
